...
class GolfCameraManager(private val context: Context) {
    ...
    companion object {
        ...
        private const val MOTION_MIN_DELTA = 10f
        private const val FRAME_DIFF_MIN_DELTA = 8f
        private const val LUMA_PERCENTILE = 0.80f
        private const val TRACKING_SEARCH_RADIUS_PX = 160f
    }
    ...
    private var prevLuma = FloatArray(0)
    ...
    private fun buildLumaMaps(yData: ByteArray, rowStride: Int, pixelStride: Int) {
        ...
        val brightnessCutoff = max((profile.brightnessThreshold - 10).toFloat(), percentileLuma)

        for (i in currentLuma.indices) {
            if (backgroundLuma[i] == 0f) backgroundLuma[i] = currentLuma[i]
            val bgDelta = abs(currentLuma[i] - backgroundLuma[i])
            val frameDelta = abs(currentLuma[i] - prevLuma[i])
            motionMask[i] = bgDelta >= MOTION_MIN_DELTA || frameDelta >= FRAME_DIFF_MIN_DELTA
            brightMask[i] = currentLuma[i] >= brightnessCutoff
            backgroundLuma[i] = backgroundLuma[i] * (1f - BG_ALPHA) + currentLuma[i] * BG_ALPHA
            prevLuma[i] = currentLuma[i]
        }
    }
    ...
    private fun detectBallCandidate(frameWidth: Int, frameHeight: Int): Candidate? {
        ...
        val predX = if (kalmanInitialized) ((kalmanX + kalmanVx) / DOWNSAMPLE) else -1f
        val predY = if (kalmanInitialized) ((kalmanY + kalmanVy) / DOWNSAMPLE) else -1f
        val trackingRadiusDown = TRACKING_SEARCH_RADIUS_PX / DOWNSAMPLE
        ...
        for (y in 1 until downHeight - 1) {
            for (x in 1 until downWidth - 1) {
                ...
                val inTrackingWindow = kalmanInitialized && hypot((x - predX).toDouble(), (y - predY).toDouble()) <= trackingRadiusDown
                val canSeed = brightMask[idx] || (inTrackingWindow && motionMask[idx])
                if (!canSeed) continue
                ...
                val blob = collectComponent(
                    ...
                    minArea = minAreaDown,
                    maxArea = maxAreaDown,
                    allowStaticNeighborhood = inTrackingWindow
                )
        ...
        val selected = when {
            kalmanInitialized -> {
                ...
                candidates.maxByOrNull { c ->
                    val distNorm = (hypot((c.cx - px).toDouble(), (c.cy - py).toDouble()) / frameWidth).toFloat()
                    c.confidence - distNorm * 0.9f
                }
            }
            hintedX != null && hintedY != null -> {
                candidates.maxByOrNull { c ->
                    val distNorm = (hypot((c.cx - hintedX).toDouble(), (c.cy - hintedY).toDouble()) / frameWidth).toFloat()
                    c.confidence - distNorm * 0.45f
                }
            }
            else -> candidates.maxByOrNull { it.confidence }
        }

        val minConfidence = if (kalmanInitialized) 0.28f else 0.40f
        return selected?.takeIf { it.confidence >= minConfidence }
    }
    ...
    private fun collectComponent(
        ...
        minArea: Float,
        maxArea: Float,
        allowStaticNeighborhood: Boolean
    ): Candidate? {
        ...
        while (stack.isNotEmpty()) {
            ...
            for (ny in y - 1..y + 1) {
                for (nx in x - 1..x + 1) {
                    ...
                    val active = brightMask[nIdx] && (motionMask[nIdx] || allowStaticNeighborhood)
